...
void main() {
  ...
  if (
    ...
    if (!reflectionHit) {
      vec3 skyMieReflection =
        ...
        7;
      vec3 skyReflection =
        calcSkyColor(reflect(normalize(viewPos), normal)) * 5;
      vec3 sunReflection =
        skyboxSun(lightVector, reflect(normalize(viewPos), normal), sunColor) *
        3;
      skyReflection = mix(sunReflection, skyReflection, 0.5);
      vec3 fullSkyReflection = mix(skyReflection, skyMieReflection, 0.5);
      reflectedColor = fullSkyReflection;
  ...
  if (canReflect || isMetal || isWater) {
    if (!inWater) {
      vec3 skyMieReflection = calcMieSky(
        normalize(reflectedViewPos),
        worldLightVector,
        sunColor,
        viewPos,
        texcoord
  ...
  if (clamp(reflectedPos.xy, 0, 1) == reflectedPos.xy && isMetal) {
    ...
    clamp(reflectedPos.xy, 0, 1) != reflectedPos.xy &&
    isMetal &&
    lightmap.g > 0.74
  ) {
    color.rgb = reflectedColor;
