...
void main() {
  ...
  if (isWater) {
    ...
    float waveIntensity = 0.237 * WAVE_INTENSITY * waveIntensityRolloff;
  ...
  float wetRoughness = mix(roughness, roughness * 0.06, wetness);
 
  roughness = mix(roughness,wetRoughness, smoothLightmap);
