...
vec3 LightVector = normalize(shadowLightPosition);
 ...
 vec3 absorption = vec3(0.7882, 0.4627, 0.2118);
  ...
  vec3 inscatteringAmount = vec3(0.0, 0.0078, 0.0314);
   vec3 inscatteringAmount2 = vec3(0.008, 0.018, 0.0714);

  if(!inWater)
	{
    if(isWater && !isNight)
    {
    vec3 absorptionFactor = exp(-absorption * WATER_FOG_DENSITY * (dist * 1.1));
    ...
    {
      vec3 absorptionFactor = exp(-absorption * WATER_FOG_DENSITY * (dist * 1.3));
    color.rgb *= absorptionFactor;
   ...
   if(inWater)
    ...
    color.rgb *= absorptionFactor;
    color.rgb += vec3(0.6471, 0.4784, 0.2824) * inscatteringAmount2 / absorption * (1.0 - absorptionFactor);
    }
    else if( isNight)
    {
    dist = dist0;
