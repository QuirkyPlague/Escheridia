...
#define DRAG_MULT 0.1 // changes how much waves pull on the water
...
float getwaves(vec2 position, int iterations) {
  float wavePhaseShift = length(position) * 2.15; // this is to avoid every octave having exactly the same phase everywhere
  float iter = 0.0; // this will help generating well distributed wave directions
  float frequency = 1.0; // frequency of the wave, this will change every iteration
  float timeMultiplier = 2.0; // time multiplier for the wave, this will change every iteration
  float weight = 0.3;// weight in final sum for the wave, this will change every iteration
  ...
  for(int i=0; i < iterations; i++) {
    ...
    // modify next octave ;
    weight = mix(weight, 1.6, 1.9);
    frequency *= 1.215 * WAVE_FREQUENCY;
