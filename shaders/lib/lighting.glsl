...
//lighting variables
const vec3 blocklightColor = vec3(1.0, 0.7373, 0.6157);
vec3 skylightColor;
vec3 sunlightColor= vec3(1.0, 0.749, 0.4627);
vec3 morningSunlightColor = vec3(0.9216, 0.4353, 0.2588);
 vec3 moonlightColor = vec3(0.2863, 0.8235, 1.0);
const vec3 nightSkyColor = vec3(0.0471, 0.0706, 0.3137);
 const vec3 morningSkyColor = vec3(0.7804, 0.5216, 0.2471);
 const vec3 ambientColor = vec3(0.08, 0.08, 0.08);
  ...
  vec3 diffuseLight;  
    //Time of day changes
     ...
     vec3 sunlight;
	   ...
	   vec3 skylight = calcSkyColor(skylightColor) * lightmap.g * NIGHT_SKY_INTENSITY;
