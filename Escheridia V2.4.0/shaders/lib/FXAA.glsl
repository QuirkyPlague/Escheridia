...
vec3 FXAA(vec3 color, sampler2D cTex, vec2 texcoord) {
  ...
  float lumaDown = rgb2luma(
    texture2DLod(cTex, texcoord + vec2(0.0, -1.0) * screenView, 0.0).rgb
  );
  float lumaUp = rgb2luma(
    texture2DLod(cTex, texcoord + vec2(0, 1) * screenView, 0.0).rgb
  );
  float lumaLeft = rgb2luma(
    texture2DLod(cTex, texcoord + vec2(-1, 0) * screenView, 0.0).rgb
  );
  ...
  if (lumaRange > max(EDGE_THRESHOLD_MIN, lumaMax * EDGE_THRESHOLD_MAX)) {
    ...
    float lumaDownLeft = rgb2luma(
      texture2DLod(cTex, texcoord + vec2(-1, -1) * screenView, 0.0).rgb
    );
    ...
    vec2 offset = isHorizontal
      ? vec2(screenView.x, 0.0)
      : vec2(0.0, screenView.y);
